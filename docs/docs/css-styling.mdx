...
To style your Popup component using css classes, you can use the className we already provide by default with component or use your own custom className:
...
`reactjs-popup` provide a `className` prop, but you need to be carful using this class in your css as we add `-content` , `-overlay` and `-arrow`, which mean you need to add style for `className-popup`,`className-overlay`,`className-arrow`:
