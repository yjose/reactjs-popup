...
Adding animation to `reactjs-popup` is simple as animating any regular `div` in your react app. we recommend using css classes and `keyframes` a to animate Popup content.
...
it's important to mention that `reactjs-popup` only support animation on open.
