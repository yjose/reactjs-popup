...
function getCoordinatesForPosition(triggerBounding, ContentBounding, position, arrow, _ref) {
	const offsetX = _ref.offsetX;


	const offsetY = _ref.offsetY;
	...
	const height = ContentBounding.height;


	const width = ContentBounding.width;
...
function calculatePosition(triggerBounding, ContentBounding, positions, arrow, _ref2, wrapperBox) {
	const offsetX = _ref2.offsetX;


	const offsetY = _ref2.offsetY;
	let bestCoords;
	let i = 0;
...
const POSITION_TYPES = ['top left', 'top center', 'top right', 'right top', 'right center', 'right bottom', 'bottom left', 'bottom center', 'bottom right', 'left top', 'left center', 'left bottom'];
...
(function (_React$PureComponent) {
	function Popup(props) {
        ...
        _this.resetScroll();
			...
			let mouseEnterDelay = _this.props.mouseEnterDelay;
			_this.timeOut = setTimeout(() => {
        return _this.openPopup();
      }, mouseEnterDelay);
		}), 'onMouseLeave', () => {
			clearTimeout(_this.timeOut);
			let mouseLeaveDelay = _this.props.mouseLeaveDelay;
			_this.timeOut = setTimeout(() => {
        return _this.closePopup();
      }, mouseLeaveDelay);
		}), 'getTooltipBoundary', () => {
			let keepTooltipInside = _this.props.keepTooltipInside;
			let boundingBox = {
				top: 0,
				left: 0,
			...
			if (typeof keepTooltipInside === 'string') {
				let selector = document.querySelector(keepTooltipInside);
...
var arrow = _this$props.arrow;

          
var position = _this$props.position;

          
var offsetX = _this$props.offsetX;

          
var offsetY = _this$props.offsetY;
...
var arrowStyle = _this$props.arrowStyle;
			let modal = _this.state.modal;
			if (modal) return;

			let helper = _this.HelperEl.getBoundingClientRect();

			let trigger = _this.TriggerEl.getBoundingClientRect();

			let content = _this.ContentEl.getBoundingClientRect();

			let boundingBox = _this.getTooltipBoundary();

			let positions = Array.isArray(position) ? position : [position]; // keepTooltipInside would be activated if the  keepTooltipInside exist or the position is Array

			if (keepTooltipInside || Array.isArray(position)) positions = _toConsumableArray(positions).concat(POSITION_TYPES);
			let cords = calculatePosition(trigger, content, positions, arrow, {
				offsetX,
				offsetY
			}, boundingBox);
...
var on = _this$props2.on;
			let modal = _this.state.modal;
			let popupContentStyle = modal ? styles.popupContent.modal : styles.popupContent.tooltip;
			let childrenElementProps = {
...
var on = _this$props3.on;

          
var trigger = _this$props3.trigger;
			let onAsArray = Array.isArray(on) ? on : [on];
...
var arrow = _this$props4.arrow;

          
var arrowStyle = _this$props4.arrowStyle;
			let modal = _this.state.modal;
	...
	_createClass(Popup, [{
		...
		value: function componentDidMount() {
			...
			let closeOnEscape = _this$props5.closeOnEscape;


			let defaultOpen = _this$props5.defaultOpen;
		...
		value: function render() {
			...
			let overlayStyle = _this$props6.overlayStyle;


			let closeOnDocumentClick = _this$props6.closeOnDocumentClick;


			let on = _this$props6.on;
