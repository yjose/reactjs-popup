...
function calculatePosition(triggerBounding, ContentBounding, positions, arrow, _ref2, wrapperBounding) {
  var offsetX = _ref2.offsetX,
      offsetY = _ref2.offsetY;
  var wrapperBox = wrapperBounding ? {
  ...
  while (i < positions.length) {
    ...
    if (contentBox.top + contentBox.height >= wrapperBox.top + wrapperBox.height || contentBox.left + contentBox.width >= wrapperBox.left + wrapperBox.width) {
      i++;
    } else {
      break;
    }
...
function (_React$PureComponent) {
  ...
  function Popup(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "setPosition", function () {
      ...
      var boundingBox = keepTooltipInside && typeof keepTooltipInside.getBoundingClientRect === "function" ? keepTooltipInside.getBoundingClientRect() : null;
...
if (process.env.NODE_ENV !== "production") {
  ...
  Popup.propTypes = {
    ...
    positions: PropTypes.arrayOf(PropTypes.oneOf(["top left", "top center", "top right", "bottom left", "bottom center", "bottom right", "right top", "right center", "right bottom", "left top", "left center", "left bottom"])),
